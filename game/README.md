...
| 3.3 Combat | `combat/` | **basics (D15) + class abilities (D21):** `combat.gd` pure formulas (weapon damage, armor floor, crit, combo, player/NPC HP+damage); `abilities.gd` runner (dash / channel / burst / buff / heal from `design.abilities`, cooldowns, costs, buff multipliers, absorb); MP + M2 special attack in `player.gd`; stun / knockback / burning / slow in `entity.gd`; dodge / block / stealth in `entities/player.gd` from `design.defence` (D23); next projectiles, weapon movesets |
