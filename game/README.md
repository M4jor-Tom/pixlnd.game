...
| 3.2 Entities | `entities/` | **player (D13) + creatures (D14):** `entity.gd` base (HP, level, hostility, step-up), `player.{tscn,gd}` walk/sprint/jump/swim/stamina, `orbit_camera.gd` SpringArm3D orbit, `creature.{tscn,gd}` idle/wander/chase/return FSM, `world/spawner.gd` gen-spawns per zone, `npc.gd` service NPC at a door (D22, unattackable); `player.gd` dodge / block / stealth bar + statuses on the player, `creature.gd` stealth aggro range + hit statuses (D23); combat roles (D26): `spawner.gd` gives every group a `design.creature-roles` role (`any-class` humanoids roll melee / ranged / mage), `creature.gd` ranged / mage keep their distance, need line of sight, wind up and fire a `combat/projectile.gd` shot that applies statuses and never hits another creature; next villagers, pet, aggro table |
| 3.3 Combat | `combat/` | **basics (D15) + class abilities (D21):** `combat.gd` pure formulas (weapon damage, armor floor, crit, combo, player/NPC HP+damage); `abilities.gd` runner (dash / channel / burst / buff / heal from `design.abilities`, cooldowns, costs, buff multipliers, absorb); MP + M2 special attack in `player.gd`; stun / knockback / burning / slow in `entity.gd`; dodge / block / stealth in `entities/player.gd` from `design.defence` (D23); `projectile.gd` shots + M1 / M2 per weapon-type from `design.movesets` in `entities/player.gd` (D24); game feel (D25): `feel.gd` plays a `design.feel` bundle per event (hit-stop on `Engine.time_scale`, camera trauma on `orbit_camera.gd`, a synthesised `AudioStreamGenerator` blip, floating damage numbers, impact flash, projectile trail, stun stars) — hooked in `player.gd` (hit / crit / kill / hurt / block / dodge / shoot / level-up / pick-up), `entity.gd` (dot numbers, stars) and `projectile.gd`; creature shots reuse the same projectile + impact bundles (D26); next hit animations |
...
| 3.7 Meta | `meta/` | `input_map.gd` builds InputMap from `keybinds.json#hybrid`; `hud.gd` HP/MP/stamina/combo/land caption (+ village, D22) + hotbar cooldown line (D21) + service toasts + block-power / stealth bars, blocking text (D23) + level-up toast pop and the buff-icon row from `design.feel` (D25; "stunned!" is now 3D stars over the head); next server/client (`godot-multiplayer`), save-data |
...
nix develop -c godot --headless -s game/combat/test_defence.gd      # c-defence-config, dodge cost / roll / i-frames / rewards, shield + guardian block, block-power, stealth fill / aggro / hit bonus / camouflage / decay, enemy-hit stun + knockback
