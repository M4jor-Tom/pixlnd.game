...
func _init() -> void:
	...
	check(p.use_class_skill(1), "key 1 fires smash (D21 runtime)")
	await _frames(12)
	var expected := Combat.after_armor(p.weapon["damage"] * 2.0 * 1.05, wolves[0].armor, design["combat"])
	check(is_equal_approx(hp0 - wolves[0].hp, expected), "smash = weapon × 2 × 1.05 after armor: %.1f vs %.1f" % [hp0 - wolves[0].hp, expected])
	check(not p.use_class_skill(1) and p.abilities.cooldowns[&"smash"] > 9.0 and p.abilities.cooldowns[&"smash"] < 9.5, "second smash blocked, cooldown 10 × 0.95 s: %s" % p.abilities.cooldowns)
